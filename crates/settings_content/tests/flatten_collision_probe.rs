...
#[test]
fn fork_settings_survive_flattened_deserialization() {
    let settings: settings_content::SettingsContent = serde_json::from_value(serde_json::json!({
        "solutions": { "root": "/workspace/solutions" },
        "solution_agent": { "ephemeral": { "max_concurrent": 7 } },
        "run_config": { "toolbar": false },
        "git_panel": { "sort_by_path": true, "commit_view": { "fetch_avatars": false } }
    }))
    .unwrap();
    assert_eq!(
        settings.solutions.unwrap().root.as_deref(),
        Some("/workspace/solutions")
    );
    assert_eq!(
        settings
            .solution_agent
    ...
    assert_eq!(settings.run_config.unwrap().toolbar, Some(false));
    let git = settings.git_panel.unwrap();
    assert_eq!(git.sort_by_path, Some(true));
    assert_eq!(git.commit_view.unwrap().fetch_avatars, Some(false));
}
