...
fn create_params_accepts_null() {
    let p: CreateSolutionParams = serde_json::from_value(serde_json::Value::Null).expect("null");
...
async fn delete_solution_removes_from_store(cx: &mut TestAppContext) {
    ...
    let response = cx
        .update(|cx| {
            ...
            let id = sol_id.0;
            cx.spawn(async move |cx| tool.run(DeleteSolutionParams { solution_id: id }, cx).await)
        })
        .await
        .expect("run task");
...
fn add_catalog_params_accepts_null() {
    let p: AddCatalogProjectParams = serde_json::from_value(serde_json::Value::Null).expect("null");
    assert!(p.name.is_empty());
...
async fn remove_member_updates_store(cx: &mut TestAppContext) {
    ...
    let response = cx
        .update(|cx| {
            ...
            cx.spawn(async move |cx| tool.run(RemoveMemberParams { member_id }, cx).await)
        })
        .await
        .expect("run task");
...
fn dispatch_action_params_accepts_null() {
    let p: DispatchActionParams = serde_json::from_value(serde_json::Value::Null).expect("null");
    assert_eq!(p.solution_id, None);
...
fn diagnostics_params_accepts_null() {
    let p: GetDiagnosticsParams = serde_json::from_value(serde_json::Value::Null).expect("null");
    assert_eq!(p.solution_id, None);
...
fn goto_definition_params_accepts_null() {
    let p: GotoDefinitionParams = serde_json::from_value(serde_json::Value::Null).expect("null");
    assert_eq!(p.solution_id, None);
    assert!(p.path.is_empty());
    assert_eq!(p.line, 0);
    assert_eq!(p.col, 0);
...
fn find_references_params_accepts_null() {
    let p: FindReferencesParams = serde_json::from_value(serde_json::Value::Null).expect("null");
    assert_eq!(p.solution_id, None);
    assert!(p.path.is_empty());
    assert_eq!(p.line, 0);
    assert_eq!(p.col, 0);
...
async fn rename_member_params_accepts_null() {
    let p: RenameMemberParams = serde_json::from_value(serde_json::Value::Null).expect("null");
    assert_eq!(p.member_id, 0);
...
fn resolve_solution_id_rejects_a_non_positive_id() {
    ...
    assert!(
        err.to_string().contains("positive numeric id"),
        "got: {err}"
    );
}
...
fn every_project_tool_accepts_an_absent_solution_id() {
    ...
    // forgetting a file.
    let list: ListFilesParams = serde_json::from_value(serde_json::json!({})).expect("fs_ops");
    ...
    let read: ReadBufferParams =
        serde_json::from_value(serde_json::json!({ "path": "src/main.rs" })).expect("buffer_ops");
