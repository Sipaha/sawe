...
use util::{ResultExt, maybe, paths::PathWithPosition};
use uuid::Uuid;
use workspace::{
...
fn main() {
    ...
    app.run(move |cx| {
        ...
        // Sawe does not use upstream update channels.
...
fn handle_open_request(request: OpenRequest, app_state: Arc<AppState>, cx: &mut App) {
    if let Some(kind) = request.kind {
        match kind {
            ...
            OpenRequestKind::SharedAgentThread { session_id: _ } => {
                // Hosted thread sharing is disabled along with the upstream AgentPanel.
                log::warn!("Shared agent links are unavailable in Sawe");
            }
    ...
    if !request.open_channel_notes.is_empty() || request.join_channel.is_some() {
        cx.spawn(async move |cx| {
            let result = maybe!(async {
                ...
                // sawe: `authenticate` is credential-gated, unlike upstream's
                // `client.connect(true, ..)`, which would open an interactive
                // zed.dev browser sign-in for a user who has no account.
...
fn watch_themes(fs: Arc<dyn fs::Fs>, cx: &mut App) {
    ...
    cx.spawn(async move |cx| {
        ...
        while let Some(paths) = events.next().await {
            for event in paths {
                // Skip the watched DIRECTORY's own events. `load_bytes` on it
                // fails with a context-free `ERROR … Is a directory (os error
                // 21)` — printed on every launch, because watching a directory
