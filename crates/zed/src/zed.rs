...
use git_ui::project_diff::BranchDiffToolbar;
...
use zed_actions::{
    About, OpenAccountSettings, OpenBrowser, OpenDocs, OpenProjectTasks,
    OpenServerSettings, OpenSettingsFile, OpenStatusPage, OpenZedUrl, Quit,
};

const DOCS_URL: &str = "https://zed.dev/docs/";
const STATUS_URL: &str = "https://status.zed.dev";
// sawe: kept so re-enabling the action below is a one-line change.
#[allow(dead_code)]
...
pub fn initialize_workspace(app_state: Arc<AppState>, cx: &mut App) {
    ...
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        // above selects into. `weak_handle()` and `project()` only read plain
        // fields off `&mut Workspace`, not the entity itself, so they're safe
        // to call while this closure still holds the borrow that
        // `SolutionBand::new` must not read through (see the module doc on
        // `solution_band`).
        let solution_band = cx.new(|cx| {
            solution_agent::solution_band::SolutionBand::new(
...
fn register_actions(
    ...
    workspace
        ...
        // sawe: upstream merch store is not a surface of this fork; the action
        // and its URL stay in tree per "disable, don't delete".
...
fn initialize_pane(
    ...
    pane.update(cx, |pane, cx| {
        pane.toolbar().update(cx, |toolbar, cx| {
            ...
            let project_diff_toolbar = cx.new(|cx| ProjectDiffToolbar::new(cx));
...
mod tests {
    ...
    fn init_test_with_state(
        ...
        cx.update(move |cx| {
            ...
            theme_settings::init(theme::LoadThemes::JustBase, cx);
            // `initialize_workspace` installs fork-local surfaces (the
            // title-bar Solution tab strip, the Solution band) that read the
            // `SolutionStore` global. Production installs it in
            // `solutions::init` long before the first window opens; the real
            // `init` needs an on-disk DB, so tests get the in-memory
            // equivalent instead.
        ...
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(SplicingVec::from(vec![".sawe".to_string()]));
            });
        });
    ...
    async fn test_reload_checks_all_workspaces_for_dirty_items(cx: &mut TestAppContext) {
        let app_state = init_test(cx);
        cx.update(init);
        // Sawe defaults to IDEA-style autosave, which saves a dirty buffer on
        // close instead of prompting. This test is about the save *prompt*, so
        // it has to opt out the way the fork's other prompt tests do.
        disable_autosave(cx);
    ...
    async fn test_reload_restores_project_windows_and_tabs(cx: &mut TestAppContext) {
        ...
        let app_state = init_test(cx);
        cx.update(init);
        // Sawe lands on the launchpad by default, so `restore_or_create_workspace`
        // restores nothing unless a test asks for session restore explicitly.
        restore_last_session_on_startup(cx);
    ...
    fn test_reload_keymaps_rebuilds_menus(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            assert!(
                ...
                "the upstream Agent Panel must stay absent in Sawe"
            );
            assert!(
