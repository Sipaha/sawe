...
impl OpenRequest {
    ...
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for original_url in request.urls {
            ...
            if let Some(server_name) = url.strip_prefix("sawe-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("sawe-dock-action://") {
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::cli_ipc_socket_in(paths::data_dir(), RELEASE_CHANNEL_NAME.as_str());
            ...
            settings::SettingsStore::update_global(cx, |store, cx| {
                store.update_user_settings(cx, |settings| {
                    settings.workspace.cli_default_open_behavior =
                        Some(settings::CliDefaultOpenBehavior::NewWindow);
                    // Sawe lands on the launchpad by default, which restores no
                    // workspace; this test is about the restore path, so ask for
                    // it explicitly.
