...
fn edit_prediction_provider_config_for_settings(cx: &App) -> Option<EditPredictionProviderConfig> {
    ...
    match provider {
        ...
        // Edit prediction (Zeta) disabled in sawe (requires Zed account).
        ...
        EditPredictionProvider::Ollama | EditPredictionProvider::OpenAiCompatibleApi => {
            ...
            if matches!(format, EditPredictionPromptFormat::Zeta(_)) {
                ...
                // requires a Zed account; sawe disables that path.
                None
            } else if format == EditPredictionPromptFormat::Sweep {
                // sawe: Sweep is NOT a cloud path despite living under the same
                // `Zed(..)` config wrapper. `sweep_prompt::request_prediction`
                // reads the user's own `ollama` / `open_ai_compatible_api`
                // settings and uses a plain http client, exactly like `fim`,
                // so a self-hosted Sweep model needs no Zed account.
