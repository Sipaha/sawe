...
pub use element::{
    ...
    SplitSide, file_status_label_color, render_breadcrumb_text,
};
pub use git::blame::{BlameRenderer, GitBlame};
pub use git::{
    DefaultDiffHunkRenderer, DiffHunkRenderer, HiddenDiffHunkRenderer,
    HiddenUnstagedDiffHunkRenderer, RenderDiffHunkControlsFn, render_diff_hunk_controls,
    set_blame_renderer,
};
...
pub use split::{DiffBlameBase, DiffStyleControls, SplittableEditor, ToggleSplitDiff};
...
use editor_settings::{GitGutterWidth, GoToDefinitionFallback, Minimap as MinimapSettings};
...
use git::blame::{BlameBaseSource, GlobalBlameRenderer};
use gpui::{
...
pub struct Editor {
    ...
    blame_base_sources: HashMap<BufferId, BlameBaseSource>,
...
pub struct GutterDimensions {
    ...
    pub git_gutter_width: Option<Pixels>,
...
impl Editor {
    ...
    fn new_internal(
        ...
        let mut editor = Self {
            ...
            blame_base_sources: HashMap::default(),
    ...
    /// Builds the right-click menu for the line-number gutter, or `None` when
    /// every entry it could offer is unavailable — a diff pane with blame
    /// turned off would otherwise pop an empty menu.
    fn gutter_context_menu(
        ...
        let blame_entry = self.gutter_blame_menu_entry(cx);
                ...
                .when(show_bookmarks, |this| {
                    this.when(separator_before_bookmarks, |this| this.separator())
                        .when(has_bookmark, |this| {
                            this.entry("Edit Bookmark", Some(EditBookmark.boxed_clone()), {
                                let weak_editor = weak_editor.clone();
                                move |window, cx| {
                                    weak_editor
                                        .update(cx, |editor, cx| {
                                            editor.edit_bookmark_at_anchor(anchor, window, cx)
                                        })
                                        .log_err();
                                }
                            })
    ...
    /// Label for the gutter context menu's blame toggle, or `None` when the
    /// editor cannot produce blame at all (no project, or a scratch buffer).
    fn gutter_blame_menu_entry(&self, cx: &App) -> Option<&'static str> {
        self.project.as_ref()?;
        if self
            .buffer
            .read(cx)
        ...
        Some(if self.show_git_blame_gutter {
            "Close Git Blame Annotations"
        } else {
    ...
    fn set_gutter_context_menu(
        ...
        let Some(context_menu) = self.gutter_context_menu(anchor, display_row, window, cx) else {
            return;
        };
    ...
    pub fn gutter_dimensions(
            ...
            && let Some(ch_advance) = cx.text_system().ch_advance(font_id, font_size).log_err()
        {
            ...
            GutterDimensions {
                ...
                git_gutter_width: match gutter_settings.git_gutter_width {
                    GitGutterWidth::Custom(width) => Some(px(*width)),
                    GitGutterWidth::Default => None,
                },
