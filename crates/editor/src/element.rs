...
impl EditorElement {
    ...
    fn configured_gutter_strip_width(line_height: Pixels, cx: &App) -> Pixels {
        match EditorSettings::get_global(cx).gutter.git_gutter_width {
            GitGutterWidth::Custom(width) => px(*width),
            GitGutterWidth::Default => Self::gutter_strip_width(line_height),
        }
    }
    ...
    fn deleted_marker_base_width(setting: GitGutterWidth, line_height: Pixels) -> Pixels {
        match setting {
            ...
            GitGutterWidth::Default => Self::deleted_hunk_marker_reach(line_height),
        }
    }
    ...
    fn git_column_width(gutter_dimensions: &GutterDimensions, line_height: Pixels) -> Pixels {
        (if gutter_dimensions.mirrored {
            gutter_dimensions
                .git_gutter_width
                .map(|width| width * Self::DELETED_MARKER_WIDTH_RATIO)
                .unwrap_or_else(|| Self::deleted_hunk_marker_reach(line_height))
        } else {
            Pixels::ZERO
        }) + gutter_dimensions
            .git_gutter_width
            .unwrap_or_else(|| Self::gutter_strip_width(line_height))
            + gutter_dimensions
    ...
    fn diff_hunk_bounds(
        ...
        let gutter_strip_width = Self::configured_gutter_strip_width(line_height, cx);
        ...
        let strip_inset = if snapshot.split_side == Some(SplitSide::Right) {
            Self::deleted_marker_base_width(
