...
/// Compatibility adapter for fork consumers that provide controls as a closure.
...
struct CustomDiffHunkRenderer {
    delegate: Arc<dyn DiffHunkRenderer>,
    controls: Option<RenderDiffHunkControlsFn>,
    force_unstaged: Option<bool>,
}

impl DiffHunkRenderer for CustomDiffHunkRenderer {
    fn render_hunk_controls(
        ...
        if let Some(controls) = &self.controls {
            controls(row, status, range, created, height, editor, window, cx)
        } else {
            self.delegate
                .render_hunk_controls(row, status, range, created, height, editor, window, cx)
        }
    }

    fn render_hunk_as_staged(&self, status: &DiffHunkStatus, cx: &App) -> bool {
        match self.force_unstaged {
            Some(true) => false,
            Some(false) => !status.has_secondary_hunk(),
            None => self.delegate.render_hunk_as_staged(status, cx),
        }
    }
}
...
impl Editor {
    ...
    #[expect(
        ...
        reason = "The upstream renderer API uses Arc; GPUI renders only on the foreground thread"
    )]
    pub fn set_render_diff_hunk_controls(
        &mut self,
        controls: RenderDiffHunkControlsFn,
        cx: &mut Context<Self>,
    ) {
        self.set_diff_hunk_renderer(
            Some(Arc::new(CustomDiffHunkRenderer {
                delegate: self.diff_hunk_renderer(),
    ...
    pub fn set_render_diff_hunks_as_unstaged(&mut self, unstaged: bool, cx: &mut Context<Self>) {
        self.set_diff_hunk_renderer(
            Some(Arc::new(CustomDiffHunkRenderer {
                delegate: self.diff_hunk_renderer(),
                controls: None,
                force_unstaged: Some(unstaged),
            })),
            cx,
        );
    }
