...
mod tests {
    ...
    async fn chat_kind_rows_are_purged_by_one_shot_migration() {
        ...
        let cleanup_index = all_migrations
            .iter()
            .position(|migration| {
                *migration == sql!(DELETE FROM console_panel_state WHERE kind = "chat";)
            })
            .expect("chat-tab cleanup migration exists");
        let up_to_cleanup = &all_migrations[..cleanup_index];
