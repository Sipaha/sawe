...
impl MultiWorkspace {
    ...
    /// Reorders the Solution tabs without changing the active workspace.
    pub fn reorder_workspaces(&mut self, from: usize, to: usize, cx: &mut Context<Self>) {
        if from == to || from >= self.held.len() || to >= self.held.len() {
            return;
        }
        self.retain_active_workspace(cx);
        let workspace = self.held.remove(from);
        self.held.insert(to, workspace);
        self.serialize(cx);
        cx.notify();
    }
