...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // sawe: locked rebrand identifier — Windows resource brand strings
    // carry "Sawe", not upstream "Zed".
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Sawe"),
        "preview" => ("app-icon-preview.ico", "Sawe Preview"),
        "nightly" => ("app-icon-nightly.ico", "Sawe Nightly"),
        _ => ("app-icon-dev.ico", "Sawe Dev"),
    };
