...
use git::{
    ...
    repository::{
        AuthorHistoryEntry, Branch, BranchesScanResult, CommitData, CommitDetails,
...
impl From<CommitDiff> for git::repository::CommitDiff {
        ...
        Self {
            files: diff
                ...
                .map(|file| git::repository::CommitFile {
                    path: file.path,
                    old_content: file.old_text.map(String::into_bytes),
                    new_content: file.new_text.map(String::into_bytes),
        ...
        for update in sorted_updates {
                ...
                .map(|(id, repo)| (*id, repo.clone()))
            {
                    ...
                    update.new_work_directory_abs_path.clone()
                {
                    ...
                    let path_changed = snapshot.work_directory_abs_path
                        != new_work_directory_abs_path
                        || update
                            .dot_git_abs_path
                            .as_ref()
                            .is_some_and(|path| *path != snapshot.dot_git_abs_path)
                        || update
                            .repository_dir_abs_path
                            .as_ref()
                            .is_some_and(|path| *path != snapshot.repository_dir_abs_path)
                        || update
                            .common_dir_abs_path
                            .as_ref()
                            .is_some_and(|path| *path != snapshot.common_dir_abs_path);
                    if path_changed
...
impl Repository {
    ...
    pub fn load_commit_range(
        ...
        self.send_job("load_commit_range", None, move |git_repo, cx| async move {
            match git_repo {
                RepositoryState::Local(LocalRepositoryState { backend, .. }) => backend
                    .load_commit_range(base, head, cx)
                    .await
                    .map(decode_commit_diff),
                RepositoryState::Remote(_) => {
    ...
    pub fn tags_containing(&mut self, sha: String) -> oneshot::Receiver<Result<Vec<SharedString>>> {
        self.send_job("tags_containing", None, move |git_repo, _cx| async move {
            match git_repo {
                RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
    ...
    pub fn load_commit_diff_against_parent(
        ...
        if parent_index <= 1 {
            return self.load_commit_diff(commit, false);
        }
        let id = self.id;
        self.send_job(
            "load_commit_diff_against_parent",
            None,
            ...
            move |state, _| async move {
                match state {
                    RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
                        ...
                        let blame_task = backend.blame_at_revision(path.clone(), revision.to_string());
...
mod tests {
    ...
    #[gpui::test]
    async fn test_metadata_rename_reinitializes_backend_without_moving_worktree(
        ...
        let repository = store.read_with(cx, |store, _| {
            store.repositories().values().next().unwrap().clone()
        });
        for (source, target, branch) in [
            ("/project/.git", "/project/.git-hidden-probe", "renamed"),
            ("/project/.git-hidden-probe", "/project/.git", "restored"),
        ] {
            fs.rename(Path::new(source), Path::new(target), Default::default())
                .await
                .unwrap();
            cx.run_until_parked();
            repository.read_with(cx, |repository, _| {
                ...
                assert_eq!(repository.dot_git_abs_path.as_ref(), Path::new(target));
                assert_eq!(
                    repository.repository_dir_abs_path.as_ref(),
                    Path::new(target)
                );
                assert_eq!(repository.common_dir_abs_path.as_ref(), Path::new(target));
            });
            // Reading new branch state exercises the reopened backend, not just
            // the snapshot's metadata path fields.
            fs.set_branch_name(Path::new(target), Some(branch));
            cx.run_until_parked();
            repository.read_with(cx, |repository, _| {
                assert_eq!(repository.branch.as_ref().unwrap().name(), branch);
            });
            store.read_with(cx, |store, _| {
                assert_eq!(store.repositories().len(), 1);
                assert_eq!(store.repositories().values().next(), Some(&repository));
            });
        }
    }
