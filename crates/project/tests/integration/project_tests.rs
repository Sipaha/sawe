...
/// S-SAR — verifies that a worktree containing a `.sawe-readonly.json`
...
async fn test_read_only_marker_recognition(cx: &mut gpui::TestAppContext) {
    ...
    fs.insert_tree(
        path!("/snapshot"),
        json!({
            ".sawe-readonly.json": "{\"base_sha\":\"deadbeef\",\"branch_template\":\"snapshot/dead\",\"created_at_unix\":1700000000,\"source_repo\":\"/src\"}",
            "src": {
                "main.rs": "fn main() {}"
            }
        }),
...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    // When max_line_length is "off", default to .sawe/settings.json
...
async fn test_git_provider_project_setting(cx: &mut gpui::TestAppContext) {
    ...
    fs.atomic_write(
        Path::new(path!("/dir/.sawe/settings.json")).to_owned(),
        "{}".into(),
    )
    .await
    .unwrap();
...
async fn test_invalid_local_tasks_shows_toast_with_doc_link(cx: &mut gpui::TestAppContext) {
    ...
    // We need to start with a valid `.sawe/tasks.json` file as otherwise the
    ...
    // Update the `.sawe/tasks.json` file with an invalid variable, so we can
    // later assert that the `Event::Toast` even is emitted.
    fs.save(
        path!("/dir/.sawe/tasks.json").as_ref(),
...
async fn the_default_scope_still_starts_one_server_per_worktree(cx: &mut gpui::TestAppContext) {
    ...
    init_test(cx);
    let fs = FakeFs::new(cx.executor());
    fs.insert_tree(path!("/member-a"), json!({ "a.rs": "" }))
        .await;
    fs.insert_tree(path!("/member-b"), json!({ "b.rs": "" }))
        .await;
...
async fn project_scoped_server_covers_every_member_worktree(cx: &mut gpui::TestAppContext) {
    ...
    init_test(cx);
    let fs = FakeFs::new(cx.executor());
    fs.insert_tree(path!("/member-a"), json!({ "pom.xml": "", "a.rs": "" }))
        .await;
    fs.insert_tree(path!("/member-b"), json!({ "pom.xml": "", "b.rs": "" }))
        .await;
