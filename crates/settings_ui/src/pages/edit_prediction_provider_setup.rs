...
pub(crate) fn render_edit_prediction_setup_page(
    ...
    let providers = [
        ...
        // sawe: the Zed (Zeta) prediction provider is dead-ended in this fork
        // (edit_prediction_registry.rs maps EditPredictionProvider::Zed to None),
        // so its setup section must not be advertised here.
...
#[allow(dead_code)] // sawe: kept so re-enabling the section above is a one-line change.
