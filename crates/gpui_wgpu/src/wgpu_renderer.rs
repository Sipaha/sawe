...
impl WgpuRenderer {
    ...
    fn record_frame(&mut self, scene: &Scene, frame_view: &wgpu::TextureView) -> Result<()> {
        let encoder = self.render_scene_into(scene, frame_view)?;
        self.resources()
            .queue
    ...
    /// Encode the shared on-screen and offscreen scene without submitting it.
    ...
    pub fn render_to_image(&mut self, scene: &Scene) -> anyhow::Result<RgbaImage> {
        ...
        let mut encoder = self.render_scene_into(scene, &target_view)?;
    ...
    pub fn render_scene(&mut self, scene: &Scene) -> anyhow::Result<()> {
        ...
        let encoder = self.render_scene_into(scene, &target_view)?;
        self.resources()
            .queue
            .submit(std::iter::once(encoder.finish()));
        Ok(())
