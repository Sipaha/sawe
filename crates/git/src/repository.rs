...
pub trait GitRepository: Send + Sync {
    ...
    fn load_commit_against_parent(
        ...
        if parent_index <= 1 {
            self.load_commit(commit, false, cx)
        } else {
            future::ready(Err(anyhow!(
...
impl GitRepository for RealGitRepository {
    ...
    fn load_commit(
        ...
        cx.background_spawn(async move {
            ...
            collect_commit_diff_files(&git, changes).await
        })
        .boxed()
    }
    ...
    fn load_commit_against_parent(
        ...
        if parent_index <= 1 {
            return self.load_commit(commit, false, cx);
        }
    ...
    fn load_commit_range(
        ...
        cx.background_spawn(async move {
            let diff_output = git
                .build_command(&["diff", "--raw", "--no-abbrev", "-z", "--no-renames"])
                .arg(&base)
                .arg(&head)
                .stdin(Stdio::null())
                .stdout(Stdio::piped())
    ...
    fn initial_graph_data(
        ...
        async move {
            ...
            // Options must precede the revision/path separator inserted by get_args.
            git_log_command.extend(extra_args);
            git_log_command.extend(log_source.get_args().into_iter().map(Cow::into_owned));
...
async fn collect_commit_diff_files<'a>(
    git: &GitBinary,
    changes: impl Iterator<Item = Result<crate::commit::CommitDiffEntry<'a>>>,
) -> Result<CommitDiff> {
...
mod tests {
    ...
    #[gpui::test]
    async fn test_blame_at_revision_accepts_revision_expressions(cx: &mut TestAppContext) {
        disable_git_global_config();
        cx.executor().allow_parking();
    ...
    async fn test_blame_at_revision(cx: &mut TestAppContext) {
        ...
        let blame_at_head = repo
            .blame_at_revision(repo_path(file_name), second_sha.clone())
            .await
            .unwrap();
        assert_eq!(
            blame_at_head
        ...
        let blame_at_first = repo
            .blame_at_revision(repo_path(file_name), first_sha.clone())
            .await
            .unwrap();
        assert_eq!(
            blame_at_first
    ...
    async fn test_load_commit_range_spans_intermediate_commits(cx: &mut TestAppContext) {
        ...
        assert_eq!(modified.old_content.as_deref(), Some(b"one\n".as_slice()));
        assert_eq!(
            modified.new_content.as_deref(),
            Some(b"one\ntwo\n".as_slice())
        );
        ...
        assert_eq!(
            added.new_content.as_deref(),
            Some(b"bee\nbuzz\n".as_slice())
        );

        let deleted = files["deleted.txt"];
        assert_eq!(deleted.old_content.as_deref(), Some(b"gone\n".as_slice()));
        assert_eq!(deleted.new_content, None);

        let last_commit_only = repository
            .load_commit(third_sha, false, cx.to_async())
            .await
            .unwrap();
