...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("sawe.exe"), p("old\\sawe.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\sawe.exe"), p("old\\bin\\sawe.exe")),
        ...
        // Copy new files
        Job::move_file(p("install\\sawe.exe"), p("sawe.exe")),
        Job::move_file(p("install\\bin\\sawe.exe"), p("bin\\sawe.exe")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("sawe.exe"),
        app_dir.join("bin\\sawe.exe"),
...
fn sawe_launch_command(app_dir: &Path, launch_arguments: &[OsString]) -> std::process::Command {
    let mut command = std::process::Command::new(app_dir.join("sawe.exe"));
...
pub(crate) fn perform_update(
    ...
    if launch {
        #[allow(clippy::disallowed_methods, reason = "doesn't run in the main binary")]
        let _child = sawe_launch_command(app_dir, launch_arguments)
            .spawn()
            .context("Failed to launch Sawe after update")?;
    }
...
mod test {
    ...
    use super::{perform_update, sawe_launch_command};

    #[test]
    fn test_sawe_launch_command_preserves_arguments() {
        let arguments = vec![
            OsString::from("--user-data-dir"),
            OsString::from(r"C:\Sawe Data"),
        ];
        let command = sawe_launch_command(Path::new(r"C:\Program Files\Sawe"), &arguments);

        assert_eq!(
            command.get_program(),
            Path::new(r"C:\Program Files\Sawe\sawe.exe").as_os_str()
        );
        assert_eq!(
