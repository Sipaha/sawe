...
mod conflict_view;
pub use git_ui_core::{
    askpass_modal, created_worktrees, file_diff_view, worktree_names, worktree_picker,
    worktree_service,
};
...
pub fn init(cx: &mut App) {
    ...
    git_ui_core::set_branch_picker_builder(
        |workspace, repository, window, cx| {
            let picker =
                cx.new(|cx| branch_picker::BranchesPopup::new(workspace, repository, window, cx));
    ...
    cx.observe_new(|workspace: &mut Workspace, _window, cx| {
        ...
        // ProjectDiff owns Sawe's branch-comparison actions and unified diff UI.
        if false {
...
fn open_file_diff(
    ...
    window.defer(cx, move |window, cx| {
        SoloDiffView::open_or_focus(
            ...
            repository,
            workspace.clone(),
            solo_diff_view::DiffOpen::Summon { focus: true },
            window,
            cx,
        )
