...
impl SerializableItem for BranchDiff {
    ...
    fn serialize(
        ...
        Some(cx.background_spawn(async move {
            db.save_diff_base(item_id, workspace_id, diff_base).await
        }))
    }
...
mod tests {
    ...
    async fn test_branch_diff_action_matches_existing_item_by_base_ref(cx: &mut TestAppContext) {
        // Action listeners on the same dispatch node run in registration order
        // and consume the action by default. Register this retained upstream
        // surface before crate::init installs Sawe's ProjectDiff handlers.
        cx.update(|cx| {
            cx.observe_new(|workspace: &mut Workspace, _, cx| {
                BranchDiff::register(workspace, cx);
            })
            .detach();
        });
        init_test(cx);
