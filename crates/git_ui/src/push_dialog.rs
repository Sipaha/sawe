...
use force_confirm::{ForcePushConfirm, ForcePushGate, force_push_gate};
...
fn check_branch_protection(work_dir: &Path, branch: &str, op_name: &str) -> Option<SharedString> {
    ...
    // `confirm_push`, which re-runs the check and — instead of a
    // platform prompt — replaces the dialog's own body with the
    // in-dialog confirmation panel (`open_force_push_confirm` →
    // `render_force_push_confirm`), which names the branch, the remote
    // and the exact git command (see `force_push_gate`), lists the
    // commits the push would destroy, and arms its confirm button only
    // after a countdown. Deciding it here instead would ask before the
    // user has committed to pushing,
...
mod tests {
    ...
    pub(super) async fn boot_repo() -> Result<(TempDir, PathBuf, PathBuf)> {
        let tmp = TempDir::new()?;
