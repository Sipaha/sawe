                ...
                (Some((new_entry_id, new_repo)), Some((old_entry_id, old_repo))) => {
                    match Ord::cmp(&new_entry_id, &old_entry_id) {
                        ...
                        Ordering::Equal => {
                            ...
                            // Metadata paths can change while the working directory
                            // and scan id stay the same (for example, a gitfile is
                            // retargeted or a renamed metadata directory is restored).
                            if new_repo.git_dir_scan_id != old_repo.git_dir_scan_id
                                ...
                                || new_repo.repository_dir_abs_path
                                    != old_repo.repository_dir_abs_path
...
async fn watch_git_dir_subdirectories(git_dir_abs_path: &Path, fs: &dyn Fs, watcher: &dyn Watcher) {
    for name in [REFS_DIR, REFTABLE_DIR] {
        watch_git_ref_directory(fs, watcher, git_dir_abs_path.join(name))
            .await
            .log_err();
    }
}
