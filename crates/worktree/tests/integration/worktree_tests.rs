...
#[cfg(any(target_os = "linux", target_os = "freebsd"))]
#[gpui::test]
async fn test_native_watcher_reports_metadata_rename_within_worktree(cx: &mut TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
    let directory = TempTree::new(json!({".git": {}}));
    let tree = Worktree::local(
    ...
    let _subscription = cx.update(|cx| {
        let updates = updates.clone();
        cx.subscribe(&tree, move |_, event: &Event, _| {
            if let Event::UpdatedGitRepositories(changes) = event {
                updates.borrow_mut().extend(changes.iter().cloned());
            }
        })
    });
    let original = directory.path().join(".git");
    let held = directory.path().join(".git-hidden-probe");
    std::fs::rename(&original, &held).unwrap();
    // The existing bare-repository discovery recognizes HEAD + config in the
    // renamed directory. Its working directory stays unchanged, but consumers
    // must still learn that the metadata path changed.
    wait_for_condition(cx, |_| {
        updates
            .borrow()
            .iter()
            .any(|update| update.dot_git_abs_path.as_deref() == Some(held.as_path()))
    })
    .await;
    tree.flush_fs_events(cx).await;
    ...
    wait_for_condition(cx, |_| {
        updates.borrow().iter().any(|update| {
            update.dot_git_abs_path.as_deref() == Some(original.as_path())
                && update.new_work_directory_abs_path.as_deref() == Some(directory.path())
        })
    })
    .await;
    tree.flush_fs_events(cx).await;
    assert_eq!(
        tree.read_with(cx, |tree, _| tree.as_local().unwrap().repositories()),
        vec![Arc::<Path>::from(directory.path())]
    );
    updates.borrow_mut().clear();
    std::fs::write(original.join("HEAD"), "ref: refs/heads/restored-metadata\n").unwrap();
    wait_for_condition(cx, |_| {
        updates
            .borrow()
            .iter()
            .any(|update| update.dot_git_abs_path.as_deref() == Some(original.as_path()))
    })
    .await;
}
