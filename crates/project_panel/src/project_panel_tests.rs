...
async fn test_expand_all_entries(cx: &mut gpui::TestAppContext) {
    ...
    assert_eq!(
        entries.len(),
        12,
        "should show all 12 entries beneath the hidden root"
    );
    assert!(entries[0].contains("v dir_1"), "dir_1 expanded");
    assert!(entries[1].contains("v nested_dir"), "nested_dir expanded");
    assert!(
