...

        {
            ...
            cx.spawn_in(window, async move |_this, cx| {
                ...
                picker_handle.update_in(cx, |picker, window, cx| {
                    ...
                    picker.delegate.default_branch = default_branch.and_then(|branch| {
                        RemoteBranchName::parse_with_remotes(&branch, &configured_remotes)
                    });
...
mod tests {
    ...
    fn remote_branch_name_splits_against_the_configured_remotes() {
        let parsed = RemoteBranchName::parse_with_remotes(
            "team/fork/main",
            &remote_names(&["origin", "team/fork"]),
        )
    ...
    fn remote_branch_name_refuses_a_ref_no_configured_remote_claims() {
        assert!(
            RemoteBranchName::parse_with_remotes("team/fork/main", &remote_names(&["origin"]))
                .is_none(),
    ...
    fn remote_branch_name_falls_back_to_the_first_slash_without_a_remote_list() {
        let parsed = RemoteBranchName::parse_with_remotes("origin/main", &[])
