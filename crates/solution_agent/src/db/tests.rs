...
async fn save_then_list_returns_inserted_rows(cx: &mut gpui::TestAppContext) {
    ...
    let in_a = db.list_for_solution(SolutionId(1)).await.unwrap();
    assert_eq!(in_a.len(), 2);
    let in_b = db.list_for_solution(SolutionId(2)).await.unwrap();
    assert_eq!(in_b.len(), 1);
    let in_c = db.list_for_solution(SolutionId(3)).await.unwrap();
...
async fn cwd_roundtrips_through_save_and_list(cx: &mut gpui::TestAppContext) {
    ...
    let listed = db.list_for_solution(SolutionId(1)).await.unwrap();
...
async fn delete_removes_row(cx: &mut gpui::TestAppContext) {
    ...
    let listing = db.list_for_solution(meta.solution_id).await.unwrap();
...
async fn update_tab_orders_clears_omitted_sessions(cx: &mut gpui::TestAppContext) {
    ...
    db.update_tab_orders(SolutionId(1), vec![m2.id])
        .await
        .unwrap();
    assert_eq!(db.list_open_tabs(SolutionId(1)).await.unwrap(), vec![m2.id]);
}
...
async fn tab_order_set_after_insert_still_works(cx: &mut gpui::TestAppContext) {
    ...
    assert_eq!(db.list_open_tabs(SolutionId(1)).await.unwrap(), vec![m.id]);
}
...
async fn reopen_session_clears_stale_tab_order(cx: &mut gpui::TestAppContext) {
    ...
    assert!(db.list_open_tabs(SolutionId(1)).await.unwrap().is_empty());
...
async fn attachments_round_trip_and_cascade(cx: &mut gpui::TestAppContext) {
    ...
    let by_sol_a = db.attachment_paths_for_solution("1".into()).await.unwrap();
    ...
    db.delete_for_solution(SolutionId(1)).await.unwrap();
    assert!(
...
async fn delete_for_solution_cascades(cx: &mut gpui::TestAppContext) {
    ...
    assert_eq!(db.list_for_solution(SolutionId(2)).await.unwrap().len(), 1);
}
...
async fn delete_for_solution_removes_rows_from_all_six_tables(cx: &mut gpui::TestAppContext) {
    ...
    db.delete_for_solution(SolutionId(4)).await.unwrap();
    ...
    assert_eq!(db.list_for_solution(SolutionId(5)).await.unwrap().len(), 1);
}
...
async fn purge_session_removes_rows_from_all_six_tables(cx: &mut gpui::TestAppContext) {
    ...
    let listed = db.list_for_solution(SolutionId(9)).await.unwrap();
...
async fn session_settings_roundtrip_and_coalesce(cx: &mut gpui::TestAppContext) {
    ...
    let listed = db.list_for_solution(SolutionId(13)).await.unwrap();
    ...
    let listed2 = db.list_for_solution(SolutionId(13)).await.unwrap();
    ...
    let listed3 = db.list_for_solution(SolutionId(13)).await.unwrap();
    let loaded3 = listed3
        .iter()
