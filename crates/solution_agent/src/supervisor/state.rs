...
// This used to return a two-field `UsageLimitEta { log, user }`: an English
// rendering for the diary line and a Russian one (hand-rolled month
// abbreviations, since chrono's `%b` is English-only) for the user-facing note.
// The product's base language is English, so both audiences now read the same
// string and the split — plus the month table — is gone.
...
pub fn format_usage_limit_eta(resume_ms: i64, now_ms: i64) -> String {
    ...
    format!("{} {time}", resume_local.format("%b %d"))
}
