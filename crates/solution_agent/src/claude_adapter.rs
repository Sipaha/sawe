...
pub(crate) fn solution_system_prompt(solution: &Solution, instruction_file: &str) -> String {
    ...
    buf.push_str(
        "\nCollaboration with other agents in this Solution:\n\
         ...
         - Delivery acceptance is not completion of the other agent's work. A running recipient receives the message through its runtime's normal delivery boundary; an eligible idle session can resume. Respect rejection for paused sessions or pending human decisions. A cold session restored after an editor restart requires a user message before peer-triggered wake; do not work around that rejection. Continue independent work instead of polling or repeatedly resending.\n\
