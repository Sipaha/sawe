...
pub(crate) use teammate_reconciler::{
    ...
    push_and_evict_transcripts, read_complete_lines_from, scan_lines_for_completions,
};
// Only reached from the inline `#[cfg(test)]` modules below (`background_agent_dir_for`
// / `PARENT_JSONL_READ_CAP`); their non-test callers live inside `teammate_reconciler`
// itself, so gate the re-export to avoid an unused-import warning in release builds.
#[cfg(test)]
pub(crate) use teammate_reconciler::{PARENT_JSONL_READ_CAP, background_agent_dir_for};
