...
- **Known residual, and it is a product decision rather than a missing guard.** `persist_main_stream` is deliberately unguarded: typing into a tab whose transcript failed to load writes from Main index 0 and trims, and in the epoch arm it also rewinds the persisted wipe marker through `save_epoch`/`save_change_seq`. Appending with a base offset does **not** fix it — `idx` tolerates a base, but a new entry's `mod_seq` comes from a counter seeded off the empty transcript, so the appended turn lands below every client cursor and is invisible for good. The shape to build is **retry the load, then refuse**: re-run the load on the first send, clear the flag on success, and refuse with a visible error on failure. Suppressing the write silently is the option to reject — it loses the user's turn with no signal, which is the same lie in the other direction.
- **The refusal is cold-path-only.** Every read RPC prefers the in-memory store, so once a Solution is open and hydrated, a corrupt session reads as an empty transcript with no error. The state is non-destructive now, not honest. The live session does carry the flag, and both regimes converge on one `build_get_session_result` call site, so surfacing it is a condition on an existing seam plus a UI decision — not a new mechanism.
