...
- **The rows-empty branch is not just "pre-Phase-4 sessions", and there is a hazard behind it that predates this path.** `build_cold_session` consults `acp_thread_blob` and bumps the epoch whenever a session has no entry rows. The bump itself is benign — the read persists nothing, so it is re-derived from the same persisted value every time and cannot compound, and a desktop restore advertises the same number. The population, however, does not only shrink: `reset_context` (`/clear`) calls `persist_all_rows`, which issues `upsert_entries_and_trim(id, [], 0)` and deletes every entry row, while nothing ever clears `acp_thread_blob` (`save_blob` has no production caller). So a legacy session that was migrated and later `/clear`ed re-enters this branch permanently, and both this path *and* `hydrate_all_for_solution` then decode the retained blob and replay the **pre-`/clear`** transcript — the exact thing `store.rs`'s comment there claims cannot happen. Not introduced here, and not a reason to stop sharing hydration's constructor, but do not read this as a bounded cost. The one-line mitigation if it is taken: in the rows-empty branch a persisted `epoch > 0` proves the session was row-native and got wiped, so the blob must not be consulted.
...
- **`blob` is the one input with no fingerprint**, and its safety therefore rests on its writers rather than on the check — so it can be weakened silently by a commit elsewhere in the crate, and one immediately did. `persist_context_wipe` (#105) gave `acp_thread_blob` its first production writer; the cache stays correct only because both wipe call sites also `bump_epoch()` and `clear_total_tokens()`, which move two independently-compared fields. **That is the door a new writer comes through — check it.**
