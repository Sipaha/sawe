...
| `assets/prompts/{content_prompt,content_prompt_v2,terminal_assistant_prompt}.hbs` | Preserve user/document language, literal paths and exact output protocols in model-neutral defaults. | Prompt audit |
| `crates/agent/src/templates/{system_prompt,experimental_system_prompt}.hbs`, `templates.rs` | Sawe assistant identity and capability-aware, evidence-based instructions. | Prompt audit |
| `crates/agent/src/thread.rs` | Preserve multiline streamed summaries instead of dropping all but the first line per chunk. | Prompt audit |
| `crates/agent_settings/src/prompts/*.txt`, `crates/git_ui/src/commit_message_prompt.txt` | Preserve user language, evidence and continuation/approval state in generation prompts. | Prompt audit |
| `script/prompts`, `script/docs-suggest`, `.factory/prompts/crash/{investigate,link-issues}.md` | Safe Sawe override links; current-repository/local-crash maintenance defaults. | Prompt audit |
...
| `crates/git_ui/src/file_diff_view.rs` | **(decision #137)** Gained the diff style controls (prev/next hunk, Unified/Split) it never had — the only `SplittableEditor` consumer the decoupling changed, because its multibuffer is a `MultiBuffer::singleton`. See #137 for why that is deliberate. | `git_ui` |
...
**Supervisor Observer nudge = ONE marked message, not a note + a bubble.** A supervisor "Continue"/nudge is delivered to the agent AS a user message (so the agent acts on it). It used to ALSO push a separate `Observer directed the agent: {gist}` Observer breadcrumb note — two conversation elements for one action. That breadcrumb is removed (`store::send_supervisor_nudge`). Instead the nudge's content block is stamped with the `spk_observer_nudge` `_meta` marker (`acp_thread::meta_with_observer_nudge` / `SPK_OBSERVER_NUDGE_META_KEY`, mirroring `spk_client_send_id`; rides on `_meta`, invisible to the agent's text and round-trips into `SessionEntry::UserMessage.chunks` verbatim). `render_user_message` detects it via `acp_thread::is_observer_nudge_blocks(chunks)` and renders the message as an Observer comment (eye plaque + `Observer` tag, accent tint + left border) instead of the plain blue user bubble. Net: the single marked message carries both the full instruction and the observer attribution. How to apply: any editor-injected "acts-as-a-user-message-but-isn't-the-human" send should use the same `_meta`-marker + renderer-detect pattern rather than a companion breadcrumb note.

**The agent-INVISIBLE observer note and the agent-VISIBLE observer nudge must be visually distinguishable** (both are "the observer speaking," but one the agent acts on and one only the operator ever sees — the maintainer reported them looking identical). The two plaques now diverge: the agent-invisible `SystemEntryLevel::Observer` note uses a **crossed-eye** icon (`IconName::EyeOff`), the tag **`Observer · you only`**, and a **dashed** left border (`border_dashed`, "not part of the agent's conversation stream"); the agent-visible nudge keeps the plain `Eye`, tag **`Observer · to the agent`**, and a solid border. How to apply: audience (agent-visible vs operator-only) is a first-class distinction in the render — don't collapse the two observer bubbles to the same chrome just because both are accent-tinted eye plaques. The debug screenshot gate for this lives in `seed_cold_session` (`mcp.rs`): `role:"observer"`/`"system"` seeds the invisible note, and the new `role:"nudge"` seeds a `UserMessage` carrying `meta_with_observer_nudge()` so both plaques can be shot side by side without a live judge cycle.

**The transcript gets the full text; the attention surfaces get the lede.** An escalation reaches the operator on THREE surfaces, and they have different budgets. The in-chat Observer bubble renders markdown and scrolls, so it keeps the supervisor's untruncated `question` / `done` reasoning. The pinned question banner (`SolutionSession.supervisor_question`, painted above the status row in `session_view.rs`) and the freedesktop toast (`notifier::dispatch_raw`) are plain-text strips with no markdown renderer and roughly two lines of room — putting the same paragraph there rendered the ask TWICE, the second time as an unreadable raw-markdown slab (`**bold**`, backticks) directly under the bubble that had just rendered it properly, and made the toast unreadable. `crate::supervisor::short_gist` (`supervisor/gist.rs`, pure + unit-tested) is the shared condenser: it flattens markdown (fences dropped, links → label, `` ` ``/`**` stripped, `_` deliberately left alone so `dev_local` survives) and keeps the first sentence — plus the second only if the pair fits `GIST_MAX_CHARS`. It is applied at the SURFACE (`escalate_to_user`, `notify_supervisor_done`, and the `Errored` toast body in `store.rs`), never at the source, so the full text still reaches the bubble, the session log and the wire. Because only the lede escapes, `supervisor_judge_instructions.md` ("Operator-facing text: a verdict, not a retelling") tells the judge to open with one sentence stating the ask or outcome.

**The observer writes a verdict, not a second copy of the agent's report.** The same prompt section fixes the other half of the complaint: the judge's `done` reasoning used to be a step-by-step recap of what the agent had just said, with "verified this myself" appended to each item — the same text twice, in the bubble directly under the agent's own message. The instructions previously *required* that ("make it a thorough, self-contained summary of what was accomplished across the WHOLE session"), justified by the durable session log. That justification was wrong: `mcp/context.rs` already appends the AGENT's own `state.md` to `.agents/<sid>/session-log.md` at every compaction, so the narrative is in that file, written by the party that did the work, before the judge ever speaks. The judge's entry is therefore the closing assessment in a few sentences. A claim that does NOT hold up is likewise not operator-facing prose: it is a `continue` with a `message` naming the gap TO THE AGENT, and reaches the operator only if the agent cannot close it.

How to apply: any new "same text on the bubble AND on a chrome strip" surface is this bug. Condense at the strip, not at the source — and if a surface has no markdown renderer, do not hand it markdown. Before asking a reviewing agent to summarize, check whether the reviewed agent's own summary is already durable somewhere; if it is, ask for the judgement instead.
...
One inherited claim moved here from the accordion paragraph below, where it never belonged: `preserve_preview` is left at the default `false`, which *would* let a real text edit inside the diff promote the preview to permanent — **but it cannot fire**. `SoloDiffView` never emits an `ItemEvent`, so `Pane::handle_item_edit` is never reached. That is load-bearing for the gesture model rather than an oversight; see #136 and `TODO.md` C7, which is the other half of the same fact.
...
**The accordion is kept, demoted to a secondary gesture:** `ProjectDiff` opens via `alt-enter` / cmd-click (`menu::SecondaryConfirm` → `open_accordion_diff`), `ctrl-shift-d` (`git::Diff`), and the overflow/context-menu **"Open All Changes"** entries.
...
- `sync_blame_sources` is the **sole writer** of both editors' `BlameBaseSource` maps and re-derives them from that declaration on every excerpt update. This is the seam, and it is the whole point: a consumer that instead wrote `Editor::set_blame_base_sources` itself would have its entries wiped by the next `sync_lhs_for_paths`, and again by the next Unified/Split toggle, which rebuilds the left pane from scratch. Right-pane sources are **not** conditional on the split existing — Unified mode has no left pane and still blames. **The trigger must not be either**, and it briefly was: the derivation ran only from the tail of `sync_lhs_for_paths`, which opens `let Some(lhs) = &self.lhs else { return }`, so in Unified mode nothing but `set_blame_base` ever wrote the right pane. Each operation that *adds* excerpts now calls `sync_blame_sources` itself, and `update_excerpts_for_path`'s Unified fast path — the one place that can excerpt a buffer the declaration has not yet been applied to — was folded into the general one so it cannot drift again. `expand_excerpts` widens existing excerpts and cannot introduce a buffer, but syncs too rather than reasoning about it. **`remove_excerpts_for_path` is the deliberate exception** and does not sync: a source whose buffer is gone can never be consulted, because `GitBlame::generate` walks the multibuffer's live buffers and looks each one's source up by id, and the next sync's `retain` against those same live ids drops it — the only cost is holding that path's `Entity<Repository>` handle until then. Do not "fix" it into conformance without that being the reason; do not read the rule as covering it either, which is why both it and `sync_blame_sources` say so in their doc comments.
- Either revision may be `None`, meaning "this side has no revision to be blamed against". `solo_diff_view` sets `lhs_revision: None` for a file the commit **added** (which is also every file of a **root** commit, since `git show --name-status` reports them all as `A` and `collect_commit_diff_files` never touches `<sha>^`), `rhs_revision: None` for one it **deleted**, and no declaration at all for a binary file, whose panes hold a `(binary file not shown)` placeholder rather than the file. Do **not** read those gates as redundant on the strength of `run_git_blame`'s error mapping, which is the trap: when the missing thing is the *path*, `git blame` answers `fatal: no such path`, and `crates/git/src/blame.rs` maps a non-zero exit whose stderr contains that to `Ok(Vec::new())` — so on an *ordinary* commit an ungated blame of the added file at `<sha>^`, or the deleted file at `<sha>`, only spends a subprocess to reach the same empty result the gate produces. When the missing thing is the *revision* it does not: a **root** commit has no `<sha>^` at all, git answers `fatal: bad revision '<sha>^'`, that matches neither `GIT_BLAME_NO_PATH` nor `GIT_BLAME_NO_COMMIT_ERROR`, so `run_git_blame` bails and `GitBlame` raises the failure as a `project::Event::Toast`. **One** toast, not one per file: `generate` accumulates the failures of every buffer it blamed, `dedup`s them, joins them with `, ` and emits a single event under the fixed `notification_id: "git-blame"`, so a root commit's diff produces one notification whose message concatenates a line per file — and only when the blame was `user_triggered`; otherwise it is a `log::debug!`. Toggling blame on by hand is exactly the triggered case. That is the half `lhs_revision: None` is holding up, and dropping the gate as "git handles it" would ship it. With the gates the miss degrades to a `None` repository lookup and `Task::ready(Ok(None))` — no annotations, no error, same as a remote project.
...
Guards: `solo_diff_view::tests::{test_both_panes_paint_blame_for_a_commit_diff, test_an_added_file_blames_only_the_right_pane, test_a_deleted_file_blames_only_the_left_pane, test_a_binary_file_blames_neither_pane}`, all asserting `GIT-BLAME-ENTRY-LEFT` / `GIT-BLAME-ENTRY-RIGHT` through `VisualTestContext::debug_bounds` — the painted tree, not the predicate, and each of them on **both** sides. One trap those cost a round to learn: an absent-side assertion is vacuous unless the fixture *would* answer the revision the gate suppresses, because `FakeGitRepository::blame_at_revision` errors on an unregistered `(revision, path)` pair rather than returning an empty `Blame`. Each of the three gates is registered at both `<sha>` and `<sha>^`, and each test was confirmed to fail with its own gate mutated out. The hoist itself is guarded one layer down, by `editor::split::tests::test_unified_mode_gets_rhs_blame_sources_on_excerpt_update`: it declares a `Blob` base on a Unified `SplittableEditor` *before* excerpting the path — the order a multi-file consumer is forced into — and asserts the right pane got sources anyway; deleting the `sync_blame_sources` call from `update_excerpts_for_path` fails it. No shipped consumer reaches that path yet (`solo_diff_view` excerpts before it declares, `commit_view` declares nothing), so this test is the only thing standing between the early return and a silent return.
...
| — with `preview_tabs.enabled: false` | `Pane::set_preview_item_id` no-ops, so the pane never has a preview item at all: `add_to_pane` falls back to a **permanent, focused** tab for every summon (both halves of the double-click row are off), and `preview_holds_a_diff` can never be true, so **every** retarget is declined — single click and arrow steps do nothing, permanently. Deliberate: with previews off there is no shared slot to keep clicking into, so every open is a deliberate tab. |
...
one-way button.** `LspLogView` does not override `Item::buffer_kind`, so it inherits
...
### 162. Prompt portability preserves protocols and context recovery preserves history

Repository-owned default model instructions are English and capability based.
They preserve the user's language and authorization; machine-readable fields,
provider transport formats and user-authored prompts remain unchanged. Prompt
paths and embedded JSON commands require separate JSON serialization and POSIX
shell quoting. Single-pass substitution must never reinterpret inserted values.
...
### 173. A runtime's control request is a question that blocks it — parse it permissively and always answer

`claude` asks the client for permission over `control_request` / `can_use_tool` and **blocks until the client replies**, before spawning the tool's shell. Two independent defects in that exchange each produced the same symptom: a tool call stuck at `running` for minutes, no prompt, no OS process behind it, no way out but restarting the agent (`docs/findings/2026-09-15-dropped-safety-hook-approval-hangs-the-agent.md`).

**Read side — a required field is a decision to hang.** `ControlRequestKind::CanUseTool` declared `tool_use_id: String` as required. The *safety-hook* variant of the frame (raised before the call is admitted, e.g. "Dangerous rm operation detected" for an `rm` whose target is a variable claude cannot prove non-empty) carries no `tool_use_id` at all — it brings `display_name` / `description` / `decision_reason` instead. Serde rejected the whole frame, `process.rs`'s reader logged `claude stdout parse error` at `warn` and dropped the line, and nobody ever replied. `#[serde(other)]` is NOT a safety net here: it catches an unknown *tag*, never a variant that fails to deserialize. Every field of that variant is now optional, `decision_reason` is a `serde_json::Value` so a richer shape cannot resurrect the same hang, and the `Other` arm **declines** instead of ignoring — declining is recoverable, silence is not.

**Write side — a wire shape needs a test against the real peer.** `InputMessage::ControlResponse` emits `{"type":"control_response","request_id":…,"response":<body>}`, and claude requires `<body>` to be the nested success envelope `{"subtype":"success","request_id":…,"response":{…}}` — the same shape it uses when answering us. `build_hook_response` hand-built that nesting; `permission_response` did not, so it emitted a flat body that claude **silently ignores**. A/B against a live claude: flat → never runs, nested → `tool_result` 0.2 s later. An allow must also carry `updatedInput` (the input claude actually executes). Consequence worth internalising: the ordinary Agent Teams gate never worked either, and nobody noticed, because its failure mode is the same silent stall. The `control_response` DESERIALIZER had already been fixed once with a comment explaining this exact nesting — the serializer three lines away stayed wrong for as long, because no test ever asserted that claude ACTED on a reply.

**Policy — full rights inside the Solution, a confirmation outside it.** `bypassPermissions` deliberately cannot auto-allow the safety class, so answering it by policy would mean silently defeating a guard against `rm -rf /*` from an empty variable. `claude_native::tool_authorization` (pure, unit-tested) resolves the flagged target statically — literal `NAME=value` assignments, `$HOME`/`~`, stopping at the first unresolvable part and keeping the longest literal **prefix**, which is enough because everything under a prefix shares its inside/outside answer — and auto-allows only when every target provably lands in a work dir. Anything unprovable goes to the operator. The ask is raised on the live `AcpThread` as an ordinary tool-call authorization attached to the call claude already streamed, so the existing Allow/Reject affordance renders it; claude's own words go on the call's `content` for the desktop and on `raw_input.reason`, which `mcp::dto` lifts into `ToolCallSummary.authorization_reason` for the phone — gated by the `tool_auth_reason` `wire_features` token, never a `wire_schema_version` bump (that gate is an equality check; see #157).

**The runtime version modulates how often it asks, which is not the same as causing the hang.** An earlier workaround pinned claude `2.1.258` because `2.1.259` "caused" constant hangs, and the rollback demonstrably stopped them — yet the reproducer hangs on `2.1.258` too. Both hold: measured A/B with a denied file under the cwd, `grep -rn needle .` raises an ask on `2.1.259` and none on `2.1.258`, while the `rm`-on-a-possibly-empty-variable ask fires on both. Since the client answered no ask correctly, the version that asks more turns ordinary work into hangs, and rolling back is a real mitigation of the symptom. Don't read a successful downgrade as a diagnosis — and don't read "it still happens on the old version" as proof the version was irrelevant.

How to apply: treat any request-shaped frame from a runtime as load-bearing. Parse every field of it as optional, answer on every path including the ones you did not anticipate, and verify the reply shape by making the real peer act on it — asserting your own JSON proves nothing about what the other end accepts.

### 174. Background-agent liveness is one predicate, keyed on the evidence

"Is there background work in flight that explains this session's silence?" was asked in three places with three inlined answers — the Observer gate (`tick_supervisor`), the "agent finished" notifier and the stuck-turn watchdog — and they agreed on every state but one: **registered but never observed**. `is_messageable()` mapped it to alive with no expiry whatsoever; `background_work_shows_liveness(quiet_secs)` applied a 15-minute cutoff measured from a snapshot that, in that state, does not exist. One managed agent whose JSONL was never tailed therefore held the Observer gate shut until the 1-hour reaper removed its map entry — session `qv09rxtm`, 39 minutes idle, the judge firing two seconds after the reap (`docs/findings/2026-09-15-background-agent-liveness-had-three-answers.md`).

**Why it was never tailed.** `ensure_background_agent_watcher` arms one `fs.watch` per session on `~/.claude/projects/<encoded-cwd>/<acp-session-id>/subagents/` and guarded itself on `session_id`. That path embeds the ACP session id, which `rotate_context`, `reset_context` and `resume_session` each replace mid-session — so from the first compaction onward the watcher watched a directory claude had abandoned. The guard now keys on the watched **path**; `TeammateWatchers` stores `(PathBuf, Task<()>)` and re-arming drops the old task. Note what did NOT need fixing: `FsWatcher::add` already tolerates a path that does not exist yet (`pending_registrations` + `poll_path_until_created`), so arming ahead of claude was always safe — a `create_dir` added on the opposite assumption was measured wrong and removed.

**Defence in depth.** `tick_background_agents` already walked every agent every 5 s but only reaped; it now also tails the JSONL of any non-terminal agent with no snapshot newer than `BACKGROUND_AGENT_TAIL_FALLBACK_SECS` (30 s). `tail_jsonl` reads forward from `last_offset`, so a re-tail with nothing new is a `stat`, and a healthy agent (refreshed by the watcher within ~200 ms) is skipped. `fs.watch` is an optimisation now, not the single source of truth.

**One predicate.** Liveness is a property of the evidence, not of the caller, so it is no longer a per-caller number. `BackgroundAgent::vouches_for_parent(now)`: terminal / killed / usage-walled ⇒ false at once; **observed** ⇒ age the snapshot's `mtime` against `BACKGROUND_AGENT_QUIET_MAX_SECS` (60 min — hardening #9's long quiet tool call is not death, and it is the same budget the reaper uses so "vouches" and "tracked" expire together); **never observed** ⇒ age `registered_at` against `BACKGROUND_AGENT_UNOBSERVED_GRACE_SECS` (5 min), because nothing has corroborated this agent yet. The short grace is only safe *because* the tick now tails every agent. `SolutionSession::has_live_background_work(now)` wraps that with the running-shell half and is what all three callers read; `background_work_shows_liveness` is gone. `is_messageable` is renamed **`transcript_is_open`** — a bookkeeping fact that never expires (tab rendering, reconnect kill-marking, the permission-mode gate), explicitly not liveness.
...
### 184. The editor's fonts are measured against IDEA, not chosen

The maintainer put the two editors side by side: Sawe's editor text was too
large. Everything below is measured at the settings both actually run with — X11,
96 DPI, `Xft.dpi 96`, `text-scaling-factor 1.0`, so IDEA's JBUIScale is 1 and its
"13pt" is 13 px.

IDEA carries no editor font in `editor-font.xml` at all, so its defaults apply:
...
setting that happens to share the number. Its UI family is Inter.)

**The font family is the setting that mattered.** The first pass kept Lilex
(`.ZedMono`) and solved for the size, which cannot converge: per 1000 em Lilex
has a cap height of 698 against JetBrains Mono's 730 and an x-height of 516
against 550, so matching cap height asked for 13.60 and matching x-height asked
for 13.86 — and *neither* could fix the character advance, since both families
are 600/1000 em and matching glyph height therefore necessarily widens the
columns. Bundling the same font removes the trade-off instead of splitting it.

So: `assets/fonts/jetbrains-mono/` carries the four styles from the official
v2.304 release (byte-identical, verified by sha256, to the copies IDEA ships in
`jbr/lib/fonts/`) plus its `OFL.txt`. SIL OFL 1.1 with **no** Reserved Font Name,
so bundling unmodified costs us only keeping the notice next to the font — which
is where `assets/fonts/lilex/OFL.txt` and `assets/fonts/ibm-plex-sans/license.txt`
already put it. `Assets::load_fonts` walks `fonts/**` for `.ttf`, so dropping the
directory in is the whole wiring. `.ZedMono` still aliases Lilex and Lilex is
still bundled.
...
sizes, so the maintainer's own settings carried `buffer_font_size: 15` because
...
The maintainer, 2026-09-23, after the session tab's age label went monospaced (#204): *«Я думаю
вообще во всем редакторе надо перейти на моношрифтовый шрифт»*.

What: `assets/settings/default.json` sets `"ui_font_family": "JetBrains Mono"` (was `.ZedSans`,
...
the agent conversation (prose, tool rows, status row), the session tab strip with its age slot
and the `+` picker all render in JetBrains Mono with no clipped or overlapping text. Mono text is
...
Follow-up the same day, *«еще сделай пункты в дропдауне повыше раза в полтора. Мелчить тут нету
смысла»*: `ui::ContextMenu::item_height(px)` gives every entry and submenu row a fixed height
instead of the dense default (~23px), and the cleanup menu uses 34px. Only this menu opts in;
every other `ContextMenu` stays dense. Guarded by `context_menu::tests::
item_height_sets_the_entry_row_height` (entries now carry a `context-menu-entry-{ix}` debug
selector; mutation-checked).
