...
- **A user drag wins, and it is re-checked rather than latched.** "`preview_widths` still equals `initial_widths`" is the test for an untouched table; while it differs the derivation keeps out of the way, and double-clicking the dividers back to their defaults hands the table back to it.
