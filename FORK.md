...
- **`read_session_history` and `get_session` now agree on every combination of rows / blob / epoch / metadata row**, including the case that used to differ for a silly reason: the archive path used `load_blob` as its existence check, so "no blob" meant "no session". With `load_cold_head` establishing existence first, both no-transcript shapes collapse into one arm and `session_not_found` is raised for a genuinely absent session only. One shape still diverges and is left alone deliberately: an **undecodable** blob makes `read_session_history` error while `build_cold_session` swallows the decode with `.ok()` and serves an empty transcript. If anyone picks that up, `get_session`'s side is the questionable one — silently serving "empty" for a corrupt on-disk transcript is how data loss gets mistaken for a wipe.
