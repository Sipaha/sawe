...
Carry collaborator provenance through delivery and queue recovery. Peer messages are visibly attributed, never fresh user authorization. Reject unsafe paused/approval states without changing them; an ordinary idle recipient may wake, and a running recipient uses its native delivery boundary. Acceptance is not evidence that the recipient completed the requested work.
