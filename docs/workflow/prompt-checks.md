...
Repeat `--case` to select a subset; omit it for all eleven. Five recovery cases cover interrupted operation effects, ambiguous handoffs, requested language, pending approval and instructions embedded in source text. Six supervisor cases cover human approval, finite asynchronous work, incomplete accepted scope, routine priority questions, consequential architecture choices, and independent work while a human decision is pending. Recovery instructions are extracted from actual Rust constants on each run. Supervisor instructions come from the actual resource with synthetic placeholders and current numeric state limits. Production prompt text is sent to the selected provider; conversations and environment paths are synthetic, with no working-session content supplied. This is an offline decision simulation: it does not validate live bridge submission, artifact updates or process recovery.
...
Each case has a wall-time limit (60 seconds by default, configurable up to 120), an output-stream limit (1 MiB per stream), and no automatic retries. Claude also receives a per-case API budget of $0.10 (configurable up to $1). Codex CLI exposes no equivalent dollar budget here: selecting cases and timeout bounds work, **not a guaranteed dollar/token cap**. Both may consume account usage. Reports retain raw synthetic answers even when strict JSON parsing fails and are written under the system temporary directory; temporary working directories are removed.
