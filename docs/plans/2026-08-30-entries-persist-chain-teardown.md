...
23. ✅ **DONE** (`solution_agent: Write an entry-row flush as one batched upsert`) — shipped as
    `SolutionAgentDb::upsert_entries_and_trim`, with the trim folded into the same lock/savepoint. Measured on one
    fixture: a 200-row flush went from **407 executor turns exposing 38 distinct partial row sets** to **7 turns and two
    states** (the same 7 a 4-row flush costs); a normal 1-row ingest persist went from 9 turns to 7. Fact 22 is
    **narrowed, not closed** — a reopen whose `load_entries` is ordered entirely *before* the flush still hydrates the
