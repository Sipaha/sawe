...
**Estimated:** 1 sub-agent session, ~3–5 h, sibling-repo dispatch (no sawe worktree)
**Goal:** Stand up `sawe-mobile` as a sibling repo of `sawe`. Land a two-module Kotlin/Gradle layout (`:core` JVM lib + `:app` Android Compose stub). The `:core` module implements the WS+TLS+HMAC handshake matching the server side that R-2 + R-3 + R-4 ship, and is verifiable with JDK alone (no Android SDK required). `:app` is a thin Compose UI surface that depends on `:core` — its files are written but it won't fully build on this machine until the Android SDK is installed.

## Context

R-4 finished the server-side surface: `remote.*` proxy over TLS+WS+HMAC, fingerprint-pinned by the QR shown in R-3. The Android client side has been arc-planned since 2026-05-15 (see [`plans/2026-05-15-remote-control.md`](2026-05-15-remote-control.md) phase R-5) but the repo didn't exist. The user just directed: place it as a sibling of `sawe`.

**Where:** `/home/spk/.spk/sawe/solutions/spk-solutions/sawe-mobile/` (sibling of `sawe`, `spk-cockpit`, `spk-mail`). The empty directory has been created by the supervisor; the sub-agent runs `git init` as part of its first commit.
...
1. **Verifiable now.** The connection layer can be unit-tested against the sawe listener with just JDK + Gradle wrapper. Without the split, every change requires a full Android SDK to even compile.
...
4. **Matches the pattern of the sawe side** — there the protocol layer (`crates/remote_control`) is independent from the UI (`crates/remote_control_ui`).

## Scope

### A. Repository init

Working directory: `sawe-mobile/` (sibling of `sawe`).

- `git init` (default branch `main`).
- Top-level files:
  - `README.md` — one-page: what this repo is, link back to the sawe `plans/2026-05-15-remote-control.md` arc, build instructions for `:core` (JDK only) vs `:app` (Android SDK needed).
  - `LICENSE` — match sawe's primary license, which is GPL-3.0-or-later for the editor crate. (Pavel Simonov is the copyright holder of fork-local code; this Android client is wholly fork-local, no upstream Zed inheritance, so GPL-3.0-or-later applies to fork-local code under the same SPDX as the editor's primary crate.)
...
```
sawe-mobile/
  settings.gradle.kts          # rootProject.name = "sawe-mobile", includes(":core", ":app")
...
- `FingerprintPinningTrustManager(expectedFp: ByteArray): X509TrustManager` — pins the server by SHA-256 of its leaf cert. Throws `CertificateException` on mismatch. Mirrors the pinning rule on the sawe side (R-3 added `server_fp` to the QR for exactly this purpose).
...
1. **Unit (no socket):**
   ...
   - `HmacChallengeAuthTest` — given a fixed secret + nonce, the computed HMAC matches the same `hmac-sha256` reference the sawe side computes (use the same test vectors that `remote_control` uses for its handshake; copy the vector values, not the implementation).
   - `JsonRpcEnvelopeTest` — request id round-trips, error envelope deserialises.

2. **Integration (against a live sawe):** in a separate test source set or behind a `@Tag("integration")` JUnit tag so the default `./gradlew :core:test` runs only unit tests. The integration test:
   1. Reads `SAWE_PAIRING_URL` env var (set manually by the dev — the live sawe listener prints this when started).
   ...
   The unit tests are the load-bearing R-5a acceptance gate. The integration test is a nice-to-have — it'll be wired up but not part of the green-build requirement, because it needs a running sawe and the dev has to wire the env var.
...
- `ui/App.kt` — Compose root: a single `Scaffold` with a top bar saying "Sawe remote" and a body that shows one of three states:
...
Don't add CI yet — the R-1..R-4 work in sawe is also CI-free per the user's working pattern (`.github/workflows/` is mostly disabled). Just write a `README.md` note that says "Local build only, no CI yet."
...
4. **Pure-JVM tests for `:core`**; integration test exists but is `@Tag("integration")`-gated. Don't wire the live sawe into the green-build gate — too easy to forget the env var and then "tests don't run" becomes the default.
5. **Package root `ru.sipaha.spkremote`** mirrors the bundle-id pattern (`ru.sipaha.sawe`) from sawe's macOS bundles. Consistent identity across the fork's user-visible identifiers.
...
Working directory: `/home/spk/.spk/sawe/solutions/spk-solutions/sawe-mobile/`.
...
- [x] `git init` + initial commit in `sawe-mobile/`.
...
- [x] `README.md` documents: how to run `:core` tests (just `./gradlew :core:test`), how to wire the integration test (`SAWE_PAIRING_URL=spk-remote://... ./gradlew :core:integrationTest`), and that `:app` needs `ANDROID_HOME` to build.
- [x] License is GPL-3.0-or-later with `Copyright (c) 2026 Pavel Simonov`.

## When done

Sub-agent reports:
- The initial-commit SHA in `sawe-mobile`.
...
- HMAC test vector chosen + where copied from on the sawe side.
...
2. Run the verification commands above end-to-end (post-merge MCP smoke isn't applicable — this isn't an sawe change).
...
sibling directory `/home/spk/.spk/sawe/solutions/spk-solutions/sawe-mobile/`,
which the supervisor has created (empty) before dispatch. No sawe
...
5. **HMAC reference vectors locked locally** via `javax.crypto.Mac` on JDK 25: `secret=32×0x42, nonce=0x00..0x0f → 3c11ddd5996bab20165bb16079e1303302bee56f1479bbebf802ba9a51980cbb`; `secret=0x00..0x1f, nonce=0xff..0xf0 → 1570e414c43bc8fdad1098ba0b3a6aec1a107d271fe6af665c737032cb0a515b`. Reproducible from the test source. The sawe side's HMAC vectors can be cross-checked against these when R-5a's integration test is wired up.
...
- `LiveEditorIntegrationTest` grew from a stub into a six-step end-to-end probe: `connect` → `remote.editor.capabilities` (assert `protocol_version`) → `remote.solutions.list` (empty allowed) → `remote.lsp.start` (assert `-32601` proving R-4 allow-list works) → `remote.editor.subscribe { kinds: [...] }` → post-`close` call must not succeed. Still `@Tag("integration")` — opt-in via `-DincludeTags=integration` + `SAWE_PAIRING_URL`, default `:core:test` keeps the test invisible.
- New `:cli` JVM module — pure-JVM smoke client over `:core`. Reads pairing URL from argv or `SAWE_PAIRING_URL`, optional JSON-RPC method + params; pretty-prints the response. `./gradlew :cli:run --args="<pairing> <method> <params>"` is the entrypoint. No-args prints usage and exits 1.

Supervisor-verified:
- `:core:test --rerun-tasks` → 30 PASSED, 0 failed. R-5a baseline preserved.
- `:cli:build` → BUILD SUCCESSFUL.
- `:core:test -DincludeTags=integration --rerun-tasks` → discovers `LiveEditorIntegrationTest > connects, probes allow-list, subscribes()`, which then SKIPS via JUnit `Assumptions.assumeTrue` because `SAWE_PAIRING_URL` isn't set in the verifier's environment. Tag gate confirmed working in both directions.
...
- **Cross-side verification** — when the live sawe server is started by the maintainer, run `./gradlew :core:test -DincludeTags=integration -DSAWE_PAIRING_URL='spk-remote://...'` to confirm the wire-level HMAC + TLS pinning round-trip works end-to-end. This is the first time sawe's R-2/R-3/R-4 surface gets exercised by an actually-independent client.
