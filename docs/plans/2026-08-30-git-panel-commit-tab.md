...
- **Content is exactly spec §5 and no more:** full commit message, `short hash · author · date`, the changed-files tree with status colouring, and **whole-commit** +/− totals in the header — mirroring what the graph's sidebar showed. ~~**Per-file +/− counts are out of scope** (`CommitFile` carries no numstat; FORK.md #55 already ruled this out once).~~ — **OVERTURNED 2026-09-02 (FORK.md #127).** The premise was wrong by the time it was written: the header's whole-commit total is itself derived client-side, by folding `line_diff(old_text, new_text)` over every file of the `CommitDiff`, so the per-file figures were already being computed and thrown away. They are now kept per `RepoPath` on `LoadedCommitDiff` and rendered on the file rows; no new load path was needed. **Ref chips and the "In N branches" line are dropped**, together with `CommitBranches` / `branches_containing` / `format_branches_containing` and its test; both remain available in the full `CommitView` (`commit_view/refs_bar.rs`, `commit_view/contains_panel.rs`). *Cost if wrong:* two pieces of information move one click away, to the same place the full sha already lives. — **OVERTURNED 2026-09-01 for the branches half only** (FORK.md #123). The maintainer asked for it back, and the priced cost understated the defect: the deleted `format_branches_containing` printed `and N more` as *text*, so on a busy repo it announced that information existed and refused to show it. The line is back with a clickable `Show all`. The **ref-chips half stands** — decoration is not containment, and nothing here renders chips.

- **No resize split inside the Commit tab.** The sidebar's files↔message drag handle and both `SplitState` types die with it. Layout is fixed: message block on top (bounded, scrollable), metadata row, then the file tree filling the rest — the order spec §5 lists. *Rules out:* persisting a split the panel's own width already constrains. — **The ordering clause is OVERTURNED 2026-09-01** (FORK.md #122): the tab now paints changed-files header → tree → message → identity → branches, mirroring the Changes tab as the maintainer asked. Its stated justification was "the order spec §5 lists", which was never a technical constraint. ~~**The no-split half stands** and is untouched: there is still no drag handle and no `SplitState`.~~ — **the no-split half is OVERTURNED too, 2026-09-02** (FORK.md #128). The maintainer asked for the resize back. There is now a drag handle between the tree and the message, and the message height is persisted per workspace. What did *not* come back is the deleted `SplitState` pair: one field, committed in `on_drag_move`.
...
- **Single click on a file row selects it; double click opens `CommitView::open_file_diff` in the centre pane** — the graph's existing gesture split (`git_graph.rs:259-276`), preserved verbatim. *Rules out:* mouse-walking the list spraying centre-pane tabs. *Cost if wrong:* it differs from the Changes tab, where a single click opens a preview tab (FORK.md #54); matching that would need a commit-aware preview path `CommitView` does not expose. — **OVERTURNED 2026-09-01** (FORK.md #125). The ruling's own justification did not hold: double-clicking N files still produced N tabs, because the dedupe key was `(sha, file)`. Double click now summons one shared diff into the pane's preview slot and single click retargets it; the "commit-aware preview path `CommitView` does not expose" turned out to need no new capability, only `replace_preview_item_id` per FORK.md #54.
...
- **Mutation-test in a worktree, never in this shared checkout.** A Task 3 reviewer mutation-tested by writing to `crates/git_graph/src/git_graph.rs` on `main` while several other agents were live. It restored cleanly, but that was luck: a concurrent commit would have baked the mutation into someone else's diff.
