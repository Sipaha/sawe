...
- **DONE** `TOOL_STUCK_SECS`=20min hard-kills legitimately long foreground tools → possible duplicate build/deploy. (fixed: the stuck watchdog now reconnects an in-progress tool past `TOOL_STUCK_SECS` only when it has ALSO shown no liveness — silent past `TOOL_OUTPUT_SILENCE_SECS`=15min AND no running OS process. For claude-acp's display-only terminals each output chunk rides a `ToolCallUpdate` that already bumps `last_activity_at`, so `silent_secs` measures output recency; a real client-side PTY's output doesn't, so `acp_thread::Terminal::is_process_running` covers it. A build/deploy that keeps printing or whose process is alive is never reconnected out from under itself; a truly-hung one still is)
...
- **DONE** Zero-output long background shells reaped as stale at ~7min → lose `has_live_background_work` suppression. (fixed: `tick_background_shells` now picks the Running-stale threshold by parent liveness — while the owning session has a live `acp_thread` a completing shell is still flipped `Exited` by the parent-JSONL scan, so a silent-Running shell is presumed alive and kept until the generous `BACKGROUND_SHELL_LIVE_PARENT_MAX_SECS`=60min cap (preserving the suppression); an orphan whose parent subprocess is gone (no `acp_thread`) still reaps at the ordinary ~7min stale+linger, and terminal shells reap immediately — so nothing leaks forever)
