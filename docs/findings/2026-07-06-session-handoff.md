...
- **Mobile:** `:core` DTOs mirror the server (tagged `StreamIdDto`, `StreamDto`); `filterEntriesBySubagent` + render-time filters DELETED; tabs from `streams`; `SUPPORTED_WIRE_SCHEMA_VERSION`→3 + `isServerTooOld` reject. `:core:test` + `:app` compile green; render verified offscreen (Roborazzi golden: Main + teammate tabs from `streams`).
