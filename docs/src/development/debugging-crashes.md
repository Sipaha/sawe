...
When Zed panics or crashes, it sends a message to a sidecar process that inspects the editor's memory and creates a [minidump](https://chromium.googlesource.com/breakpad/breakpad/+/master/docs/getting_started_with_breakpad.md#the-minidump-file-format) in `~/.spk/sawe/logs`. You can use this minidump to generate backtraces for all thread stacks.
...
```sh
zstd -d ~/.spk/sawe/logs/<session-id>.dmp -o minidump.dmp
