...
You can confirm that Zeta is properly configured by opening the [Settings Editor](sawe://settings/edit_predictions.providers) (`Cmd+,` on macOS or `Ctrl+,` on Linux/Windows) and searching for `edit_predictions`. The `provider` field should be set to `Zed AI`.
...
{
  "edit_predictions": {
    "disabled_globs": ["~/.spk/sawe/config/settings.json"]
  }
}
```
