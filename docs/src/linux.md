...
If Sawe was installed using the default installation script, it can be uninstalled by supplying the `--uninstall` flag to the `sawe` shell command:

```sh
sawe --uninstall
```

Note that this will uninstall the Sawe variant that the symlink points to. If you have multiple, parallel installations (such as both Stable and Preview), then you should instead use its absolute path as described further down in this section.
...
The first case might fail if a symlink was not properly established between `$HOME/.local/bin/sawe` and `$HOME/.local/sawe.app/bin/sawe`, or if the symlink was overwritten by having parallel installations of different versions of Sawe. But the second command should always work as long as Sawe was installed to its default location.

If Sawe was installed to a different location (such as the `sawe-preview.app` variant), you must invoke the `sawe` binary stored in that installation directory and pass the `--uninstall` flag to it in the same format as the previous commands.
