...
Strip debug symbols before upload to keep the remote server small:

```bash
llvm-objcopy --strip-debug target/release/remote_server
```

Use `llvm-objcopy`; GNU `objcopy` may not support newer LLVM CREL sections. For a static Linux server, use the target and flags in this fork's `script/bundle-linux`.
