...
## Sawe Changes and Commit Views

The Git panel follows the active Solution member's repository. **Changes** shows
the working tree and staging controls. **Commit** reviews a selected commit or
comparison, keeping that selection separate from uncommitted changes.

Working-tree file diffs are editable; historic commit file diffs are read-only.
Double-click a file to open its preview, or press Enter to open and focus it.
Single-click and arrow navigation retarget an existing preview. Open Git history
in the Solution band's utility area to select commits.
